...
public class StudentController {
    ...
    public ResponseEntity<Result<PageResult<Student>>> getStudentsPage(
            @RequestParam(defaultValue = "1") int page,
            @RequestParam(defaultValue = "10") int size,
            @RequestParam(defaultValue = "id") String sortBy,
            @RequestParam(defaultValue = "asc") String sortDir) {
        PageResult<Student> pageResult = studentService.getStudentsPage(page, size, sortBy, sortDir);
        return ResponseEntity.ok(Result.success("查询成功", pageResult));
    }
}
